...
<img src="https://i.ibb.co/JKh8W7b/gotovy3.png" alt="" width="50" height="50">
</a>
<ul class="navbar-nav" >
<li class="nav-item">
<a style="color:white" class="nav-link btn @if(\Route::current()->getName()=='about') btn-success @else btn-secondary @endif" href="{{route('about')}}" >О конкурсe</a>
</li>
<li class="nav-item">
<a style="color:white" class="nav-link btn @if(\Route::current()->getName()=='competitions') btn-success @else btn-secondary @endif" href="{{route('competitions')}}">Направления</a>
</li>
<li class="nav-item">
<a @if(!Auth::check()) class="btn btn-secondary disabled" @else class="nav-link btn @if(\Route::current()->getName() == Auth::user()->role->slug.'.home') btn-success @else btn-secondary @endif" href="{{ route(Auth::user()->role->slug.'.home')}}">{{ __('Личный кабинет') }}</a>
</li>
<li class="nav-item">
<a style="color:white" @if(!Auth::check()) class="btn btn-secondary disabled" @else class="nav-linkbtn @if(\Route::current()->getName() == 'user.profile') btn-success @else btn-secondary @endif">{{ __('Профиль') }}</a>
</li>
