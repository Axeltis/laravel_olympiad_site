...
Route::middleware('verified')->group(function () {
        ...
        Route::get('/user/home', [App\Http\Controllers\UserController::class, 'index'])->name('user.home')->middleware('access.route:view,user');
        Route::get('/user/{user_id}/edit_page', [App\Http\Controllers\UserController::class, 'editUser'])->name('user.edit_user_page');
        Route::post('/user/{user_id}/edit', [App\Http\Controllers\UserController::class, 'editUser'])->name('user.edit_user');
        // Route::delete('/user/{user_id}/delete', [App\Http\Controllers\AdminController::class, 'deleteUser'])->name('user.delete_user');
        Route::post('/user/join/competition/{competition_id}', [App\Http\Controllers\UserController::class, 'joinCompetition'])->name('user.join_competition');
        Route::post('/user/leave/competition/{competition_id}', [App\Http\Controllers\UserController::class, 'leaveCompetition'])->name('user.leave_competition');
        Route::post('/user/uploadAnswer/{holding_id}', [App\Http\Controllers\UserController::class, 'uploadAnswer'])->name('user.upload_answer');



        Route::get('/user/profile/{user_id}', [App\Http\Controllers\UserController::class, 'profile'])->name('user.profile')->middleware('access.route:view-profile');
        Route::get('/competition/{competition_id}/teaching_materials', [App\Http\Controllers\CompetitionsController::class, 'teachingMaterial'])->name('competition.teaching_materials');
